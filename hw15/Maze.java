 ...
 * Using a depth-first and backtracking algorithm similar to that of Knight's Tour, the algorithm is: Solve the maze from the next spot, if the current spot is not a disqualifier for the maze can be visited.
 ***/
...
class MazeSolver 
{
   ...
   *********************************************/
	...
	//other base case(s)...
	else if ( x < 0 || y < 0 || x >= maze.length || y >= maze[x].length ) {
	    System.out.println(this);
	    return;
	}
	  ...
	  solve(x, y + 1);
	  solve(x, y - 1);
	//if it has made it this far, path did not lead to tour, so back up
	  maze[x][y] = PATH;
	  System.out.println(this);
      }
  }
